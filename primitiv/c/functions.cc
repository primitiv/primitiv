...
primitiv_Status _NODE_OP(op_name, _node_const)( \
  ...
  try { \
    *y = to_c_ptr_from_value(primitiv::functions::op_name(*to_cpp_ptr(x), k)); \
    return ::primitiv_Status::PRIMITIV_OK; \
  } HANDLE_EXCEPTION \
} \
primitiv_Status _NODE_OP(op_name, _const_node)( \
  ...
  try { \
    *y = to_c_ptr_from_value(primitiv::functions::op_name(k, *to_cpp_ptr(x))); \
    return ::primitiv_Status::PRIMITIV_OK; \
  } HANDLE_EXCEPTION \
} \
primitiv_Status _NODE_OP(op_name, _node_node)( \
  ...
  try { \
    *c = to_c_ptr_from_value( \
        primitiv::functions::op_name(*to_cpp_ptr(a), *to_cpp_ptr(b))); \
    return ::primitiv_Status::PRIMITIV_OK; \
  } HANDLE_EXCEPTION \
} \
...
primitiv_Status primitiv_tensor_func_input(
  ...
  try {
    *tensor = to_c_ptr_from_value(
        primitiv::functions::input_tensor(*to_cpp_ptr(shape),
...
primitiv_Status primitiv_node_func_copy(
  ...
  try {
    *y = to_c_ptr_from_value(primitiv::functions::copy(*to_cpp_ptr(x),
                                                   to_cpp_ptr(dev)));
    return ::primitiv_Status::PRIMITIV_OK;
  } HANDLE_EXCEPTION
}
...
primitiv_Status primitiv_node_func_matmul(
    const primitiv_Node *a, const primitiv_Node *b, primitiv_Node **c) {
  try {
    *c = to_c_ptr_from_value(primitiv::functions::matmul(*to_cpp_ptr(a),
                                                     *to_cpp_ptr(b)));
    return ::primitiv_Status::PRIMITIV_OK;
  } HANDLE_EXCEPTION
}
