...
PRIMITIV_C_API primitiv_Status primitiv_node_func_##name( \
    const primitiv_Node *x, primitiv_Node **y); \
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_##name( \
    const primitiv_Tensor *x, primitiv_Tensor **y);

#define PRIMITIV_C_DECL_BINARY_OP(name) \
PRIMITIV_C_API primitiv_Status \
...
PRIMITIV_C_API primitiv_Status primitiv_node_func_matmul(
    const primitiv_Node *a, const primitiv_Node *b, primitiv_Node **c);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_matmul(
    const primitiv_Tensor *a, const primitiv_Tensor *b, primitiv_Tensor **c);
...
PRIMITIV_C_API primitiv_Status primitiv_node_func_prelu(
    const primitiv_Node *x, float a, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_prelu(
    const primitiv_Tensor *x, float a, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_elu(
    const primitiv_Node *x, float a, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_elu(
    const primitiv_Tensor *x, float a, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_sum(
    const primitiv_Node *x, uint32_t dim, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_sum(
    const primitiv_Tensor *x, uint32_t dim, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_broadcast(
    const primitiv_Node *x, uint32_t dim, uint32_t size, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_broadcast(
    const primitiv_Tensor *x, uint32_t dim, uint32_t size, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_logsumexp(
    const primitiv_Node *x, uint32_t dim, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_logsumexp(
    const primitiv_Tensor *x, uint32_t dim, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_log_softmax(
    const primitiv_Node *x, uint32_t dim, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_log_softmax(
    const primitiv_Tensor *x, uint32_t dim, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_softmax(
    const primitiv_Node *x, uint32_t dim, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_softmax(
    const primitiv_Tensor *x, uint32_t dim, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_softmax_cross_entropy(
...
PRIMITIV_C_API primitiv_Status primitiv_node_func_batch_sum(
    const primitiv_Node *x, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_batch_sum(
    const primitiv_Tensor *x, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_constant(
    const primitiv_Shape *shape, float k, primitiv_Device *dev,
    primitiv_Graph *g, primitiv_Node **node);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_constant(
    const primitiv_Shape *shape, float k, primitiv_Device *dev,
    primitiv_Tensor **tensor);

PRIMITIV_C_API primitiv_Status primitiv_node_func_identity(
    uint32_t size, primitiv_Device *dev, primitiv_Graph *g,
    primitiv_Node **node);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_identity(
    uint32_t size, primitiv_Device *dev, primitiv_Tensor **tensor);

PRIMITIV_C_API primitiv_Status primitiv_node_func_random_bernoulli(
    const primitiv_Shape *shape, float p, primitiv_Device *dev,
    primitiv_Graph *g, primitiv_Node **node);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_random_bernoulli(
    const primitiv_Shape *shape, float p, primitiv_Device *dev,
    primitiv_Tensor **tensor);

PRIMITIV_C_API primitiv_Status primitiv_node_func_random_uniform(
    const primitiv_Shape *shape, float lower, float upper, primitiv_Device *dev,
    primitiv_Graph *g, primitiv_Node **node);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_random_uniform(
    const primitiv_Shape *shape, float lower, float upper, primitiv_Device *dev,
    primitiv_Tensor **tensor);

PRIMITIV_C_API primitiv_Status primitiv_node_func_random_normal(
    const primitiv_Shape *shape, float mean, float sd, primitiv_Device *dev,
    primitiv_Graph *g, primitiv_Node **node);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_random_normal(
    const primitiv_Shape *shape, float mean, float sd, primitiv_Device *dev,
    primitiv_Tensor **tensor);

PRIMITIV_C_API primitiv_Status primitiv_node_func_random_log_normal(
    const primitiv_Shape *shape, float mean, float sd, primitiv_Device *dev,
    primitiv_Graph *g, primitiv_Node **node);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_random_log_normal(
    const primitiv_Shape *shape, float mean, float sd, primitiv_Device *dev,
    primitiv_Tensor **tensor);

PRIMITIV_C_API primitiv_Status primitiv_node_func_random_gumbel(
    const primitiv_Shape *shape, float mu, float beta, primitiv_Device *dev,
    primitiv_Graph *g, primitiv_Node **node);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_random_gumbel(
    const primitiv_Shape *shape, float mu, float beta, primitiv_Device *dev,
    primitiv_Tensor **tensor);

PRIMITIV_C_DECL_BINARY_OP(pow);

PRIMITIV_C_API primitiv_Status primitiv_node_func_ipow(
    const primitiv_Node *x, uint32_t k, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_ipow(
    const primitiv_Tensor *x, uint32_t k, primitiv_Tensor **y);

PRIMITIV_C_DECL_UNARY_FUNC(selu);

PRIMITIV_C_API primitiv_Status primitiv_node_func_sum_nodes(
    const primitiv_Node *const *xs, size_t n, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_sum_tensors(
    const primitiv_Tensor *const *xs, size_t n, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_mean(
    const primitiv_Node *x, uint32_t dim, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_mean(
    const primitiv_Tensor *x, uint32_t dim, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_mean_nodes(
    const primitiv_Node *const *xs, size_t n, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_mean_tensors(
    const primitiv_Tensor *const *xs, size_t n, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_batch_mean(
    const primitiv_Node *x, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_batch_mean(
    const primitiv_Tensor *x, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_batch_normalize(
    const primitiv_Node *x, primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_batch_normalize(
    const primitiv_Tensor *x, primitiv_Tensor **y);

PRIMITIV_C_API primitiv_Status primitiv_node_func_zeros(
    const primitiv_Shape *shape, primitiv_Device *dev, primitiv_Graph *g,
    primitiv_Node **node);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_zeros(
    const primitiv_Shape *shape, primitiv_Device *dev,
    primitiv_Tensor **tensor);

PRIMITIV_C_API primitiv_Status primitiv_node_func_ones(
    const primitiv_Shape *shape, primitiv_Device *dev, primitiv_Graph *g,
    primitiv_Node **node);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_ones(
    const primitiv_Shape *shape, primitiv_Device *dev,
    primitiv_Tensor **tensor);

PRIMITIV_C_API primitiv_Status primitiv_node_func_dropout(
    const primitiv_Node *x, float rate, PRIMITIV_C_BOOL enabled,
    primitiv_Node **y);
PRIMITIV_C_API primitiv_Status primitiv_tensor_func_dropout(
