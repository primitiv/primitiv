...
#define MESSAGE(status) if (err == status) return #status
  MESSAGE(CUDNN_STATUS_SUCCESS);
  MESSAGE(CUDNN_STATUS_NOT_INITIALIZED);
  MESSAGE(CUDNN_STATUS_ALLOC_FAILED);
  MESSAGE(CUDNN_STATUS_BAD_PARAM);
  MESSAGE(CUDNN_STATUS_INTERNAL_ERROR);
  MESSAGE(CUDNN_STATUS_INVALID_VALUE);
  MESSAGE(CUDNN_STATUS_ARCH_MISMATCH);
  MESSAGE(CUDNN_STATUS_MAPPING_ERROR);
  MESSAGE(CUDNN_STATUS_EXECUTION_FAILED);
  MESSAGE(CUDNN_STATUS_NOT_SUPPORTED);
  MESSAGE(CUDNN_STATUS_LICENSE_ERROR);
  MESSAGE(CUDNN_STATUS_RUNTIME_PREREQUISITE_MISSING);
  MESSAGE(CUDNN_STATUS_RUNTIME_IN_PROGRESS);
  MESSAGE(CUDNN_STATUS_RUNTIME_FP_OVERFLOW);
#undef MESSAGE
  return "Unknown cudnnStatus_t value.";
}
