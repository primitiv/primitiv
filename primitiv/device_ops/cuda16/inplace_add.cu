...
__global__ void inplace_add_dev(
  ...
  const std::uint32_t batch_pos = blockIdx.y * size;

  const float x = __half2float(px[i + mbx * batch_pos]);
  
  const std::uint32_t iy = i + mby * batch_pos;
...
void CUDA16::inplace_add_impl(const Tensor &x, Tensor &y) {
  ...
  ::inplace_add_dev<<<dim3(g1, bs, 1), dim1_x_>>>(
      CDATA(half, x),
      size, x.shape().has_batch(), y.shape().has_batch(),
      MDATA(half, y));
}
