...
OPENCLDEV_KERNEL_FW_X_CONST(divide_const_r, px[i] / k, GROUP_SIZE)
OPENCLDEV_KERNEL_FW_X_CONST(divide_const_l, k / px[i], GROUP_SIZE)
OPENCLDEV_KERNEL_BW_X_CONST(divide_const_r, pgy[i] / k, GROUP_SIZE)
OPENCLDEV_KERNEL_BW_X_CONST(divide_const_l, -py[i] * pgy[i] / px[i], GROUP_SIZE)
OPENCLDEV_KERNEL_FW_X_SCALAR_R(divide_scalar_r, inline_div, GROUP_SIZE)
OPENCLDEV_KERNEL_FW_X_SCALAR_L(divide_scalar_l, inline_div, GROUP_SIZE)
OPENCLDEV_KERNEL_FW_AB(divide, inline_div, GROUP_SIZE)

kernel __attribute__((reqd_work_group_size(GROUP_SIZE, 1, 1)))
void divide_bw_kernel(
