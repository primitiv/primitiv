...
from primitiv._tensor cimport _Tensor, wrapTensorWithNew, CppTensor
...
class _tensor_operators:
    ...
    def input(data, shape = None, _Device device = None):
        ...
        if isinstance(data[0], (float, int)):
            ...
            if shape is None:
                shape = _Shape(data[0].shape, len(data))
            data_vector = ndarrays_to_vector(data)
        return wrapTensorWithNew(new CppTensor(Tensor_input_vector(normShape(shape).wrapped, data_vector, device.wrapped[0])))

    @staticmethod
    def parameter(_Parameter param):
        return wrapTensorWithNew(new CppTensor(Tensor_parameter(param.wrapped[0])))

    @staticmethod
    def copy(_Tensor x, _Device device = None):
        if device is None:
            device = _Device.get_default()
        return wrapTensorWithNew(new CppTensor(op_copy(x.wrapped[0], device.wrapped[0])))

    @staticmethod
    def pick(_Tensor x, vector[unsigned] ids, unsigned dim):
        return wrapTensorWithNew(new CppTensor(op_pick(x.wrapped[0], ids, dim)))

    @staticmethod
    def slice(_Tensor x, unsigned dim, unsigned lower, unsigned upper):
        return wrapTensorWithNew(new CppTensor(op_slice(x.wrapped[0], dim, lower, upper)))
    ...
    def concat(xs, unsigned dim):
        ...
        return wrapTensorWithNew(new CppTensor(op_concat(vec, dim)))

    @staticmethod
    def reshape(_Tensor x, _Shape new_shape):
        return wrapTensorWithNew(new CppTensor(op_reshape(x.wrapped[0], new_shape.wrapped)))

    @staticmethod
    def flatten(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_flatten(x.wrapped[0])))

    @staticmethod
    def transpose(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_transpose(x.wrapped[0])))

    @staticmethod
    def matmul(_Tensor a, _Tensor b):
        return wrapTensorWithNew(new CppTensor(op_matmul(a.wrapped[0], b.wrapped[0])))

    @staticmethod
    def sqrt(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_sqrt(x.wrapped[0])))

    @staticmethod
    def exp(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_exp(x.wrapped[0])))

    @staticmethod
    def log(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_log(x.wrapped[0])))

    @staticmethod
    def pow(x, k):
        if isinstance(x, _Tensor) and isinstance(k, (int, float)):
            return wrapTensorWithNew(new CppTensor(op_pow((<_Tensor> x).wrapped[0], <float> k)))
        elif isinstance(x, (int, float)) and isinstance(k, _Tensor):
            return wrapTensorWithNew(new CppTensor(op_pow(<float> x, (<_Tensor> k).wrapped[0])))
        elif isinstance(x, _Tensor) and isinstance(k, _Tensor):
            return wrapTensorWithNew(new CppTensor(op_pow((<_Tensor> x).wrapped[0], (<_Tensor> k).wrapped[0])))
        else:
            raise TypeError("Argument 'x' or 'k' has incorrect type (Node or float or int)")

    @staticmethod
    def tanh(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_tanh(x.wrapped[0])))

    @staticmethod
    def sigmoid(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_sigmoid(x.wrapped[0])))

    @staticmethod
    def softplus(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_softplus(x.wrapped[0])))

    @staticmethod
    def sin(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_sin(x.wrapped[0])))

    @staticmethod
    def cos(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_cos(x.wrapped[0])))

    @staticmethod
    def tan(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_tan(x.wrapped[0])))

    @staticmethod
    def relu(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_relu(x.wrapped[0])))

    @staticmethod
    def lrelu(_Tensor x):
        return wrapTensorWithNew(new CppTensor(op_lrelu(x.wrapped[0])))

    @staticmethod
    def prelu(_Tensor x, float a):
        return wrapTensorWithNew(new CppTensor(op_prelu(x.wrapped[0], a)))

    @staticmethod
    def elu(_Tensor x, float a):
        return wrapTensorWithNew(new CppTensor(op_elu(x.wrapped[0], a)))

    @staticmethod
    def selu(_Tensor x, float a, float s):
        return wrapTensorWithNew(new CppTensor(op_selu(x.wrapped[0], a, s)))
    ...
    def sum(x, dim = None):
        ...
        if isinstance(x, list):
            ...
            return wrapTensorWithNew(new CppTensor(Tensor_sum_container(xs)))
        else:
            return wrapTensorWithNew(new CppTensor(Tensor_sum((<_Tensor> x).wrapped[0], <unsigned> dim)))
    ...
    def mean(x, dim = None):
        ...
        if isinstance(x, list):
            for t in x:
                xs.push_back(t.wrapped[0])
            return wrapTensorWithNew(new CppTensor(Tensor_mean_container(xs)))
        else:
            return wrapTensorWithNew(new CppTensor(Tensor_mean((<_Tensor> x).wrapped[0], <unsigned> dim)))

    @staticmethod
    def broadcast(_Tensor x, unsigned dim, unsigned size):
        return wrapTensorWithNew(new CppTensor(op_broadcast(x.wrapped[0], dim, size)))

    @staticmethod
    def logsumexp(_Tensor x, unsigned dim):
        return wrapTensorWithNew(new CppTensor(op_logsumexp(x.wrapped[0], dim)))

    @staticmethod
    def log_softmax(_Tensor x, unsigned dim):
        return wrapTensorWithNew(new CppTensor(op_log_softmax(x.wrapped[0], dim)))

    @staticmethod
    def softmax(_Tensor x, unsigned dim):
        return wrapTensorWithNew(new CppTensor(op_softmax(x.wrapped[0], dim)))

    @staticmethod
    def softmax_cross_entropy(_Tensor x, t, unsigned dim):
        if isinstance(t, _Tensor):
            return wrapTensorWithNew(new CppTensor(op_softmax_cross_entropy(x.wrapped[0], (<_Tensor> t).wrapped[0], dim)))
        elif isinstance(t, list):
            return wrapTensorWithNew(new CppTensor(op_softmax_cross_entropy(x.wrapped[0], <vector[unsigned]> t, dim)))
        else:
            raise TypeError("Argument 't' has incorrect type (list or Node)")
    ...
    def constant(shape, float k, _Device device = None):
        if device is None:
            device = _Device.get_default()
        return wrapTensorWithNew(new CppTensor(op_constant[CppTensor](normShape(shape).wrapped, k, device.wrapped[0])))

    @staticmethod
    def zeros(shape, _Device device = None):
        if device is None:
            device = _Device.get_default()
        return wrapTensorWithNew(new CppTensor(op_zeros[CppTensor](normShape(shape).wrapped, device.wrapped[0])))

    @staticmethod
    def ones(shape, _Device device = None):
        if device is None:
            device = _Device.get_default()
        return wrapTensorWithNew(new CppTensor(op_ones[CppTensor](normShape(shape).wrapped, device.wrapped[0])))

    @staticmethod
    def identity(unsigned size, _Device device = None):
        if device is None:
            device = _Device.get_default()
        return wrapTensorWithNew(new CppTensor(op_identity[CppTensor](size, device.wrapped[0])))

    class batch:
        @staticmethod
        def sum(_Tensor x):
            return wrapTensorWithNew(new CppTensor(op_batch_sum[CppTensor](x.wrapped[0])))

        @staticmethod
        def mean(_Tensor x):
            return wrapTensorWithNew(new CppTensor(op_batch_mean[CppTensor](x.wrapped[0])))

        @staticmethod
        def normalize(_Tensor x):
            return wrapTensorWithNew(new CppTensor(op_batch_normalize[CppTensor](x.wrapped[0])))
    ...
    class random:
        ...
        def bernoulli(shape, float p, _Device device = None):
            if device is None:
                device = _Device.get_default()
            return wrapTensorWithNew(new CppTensor(op_random_bernoulli[CppTensor](normShape(shape).wrapped, p, device.wrapped[0])))

        @staticmethod
        def uniform(shape, float lower, float upper, _Device device = None):
            if device is None:
                device = _Device.get_default()
            return wrapTensorWithNew(new CppTensor(op_random_uniform[CppTensor](normShape(shape).wrapped, lower, upper, device.wrapped[0])))

        @staticmethod
        def normal(shape, float mean, float sd, _Device device = None):
            if device is None:
                device = _Device.get_default()
            return wrapTensorWithNew(new CppTensor(op_random_normal[CppTensor](normShape(shape).wrapped, mean, sd, device.wrapped[0])))

        @staticmethod
        def log_normal(shape, float mean, float sd, _Device device = None):
            if device is None:
                device = _Device.get_default()
            return wrapTensorWithNew(new CppTensor(op_random_log_normal[CppTensor](normShape(shape).wrapped, mean, sd, device.wrapped[0])))

        @staticmethod
        def gumbel(shape, float mu, float beta, _Device device = None):
            if device is None:
                device = _Device.get_default()
            return wrapTensorWithNew(new CppTensor(op_random_gumbel[CppTensor](normShape(shape).wrapped, mu, beta, device.wrapped[0])))

    @staticmethod
    def dropout(_Tensor x, float rate, bool enabled):
        return wrapTensorWithNew(new CppTensor(op_dropout(x.wrapped[0], rate, enabled)))
